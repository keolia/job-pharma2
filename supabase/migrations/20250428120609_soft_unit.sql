/*
  # Verify and update SMTP configuration
  
  1. Changes
    - Add proper SMTP settings verification
    - Set correct SMTP configuration for Resend
    - Update email template settings
*/

-- First, verify current SMTP settings
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM auth.config 
    WHERE smtp_host != 'smtp.resend.com' 
    OR smtp_port != 465
    OR smtp_user != 'resend'
  ) THEN
    -- Update SMTP configuration with verified settings
    UPDATE auth.config
    SET
