/*
  # Fix email rate limit and SMTP configuration
...
-- Update SMTP and email configuration
UPDATE auth.config
SET
  smtp_max_frequency = 300, -- Increase rate limit to 5 minutes
...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE auth.config ADD COLUMN additional_redirect_urls text[];
  END IF;
END $$;
